...
class PRMLossLinearHead(nn.Module):
    """
    Process Reward Model Loss with Linear Head
    """
    ...
    def forward(self, inputs: torch.Tensor, rewards: torch.Tensor, labels: torch.Tensor, *, return_acc: bool = False):
        placeholder_mask = inputs == self.placeholder_token_id
        rewards = rewards[placeholder_mask]
        labels = labels[placeholder_mask]

        if labels.dtype == torch.float:
            # soft label
            assert len(self.reward_token_ids) == 2, "reward_token_ids should have 2 tokens for soft labels"
            positive_labels = labels.to(rewards.dtype)
        ...
        loss = self.loss(rewards, labels.to(rewards))
        if not return_acc:
            return loss

        acc = ((nn.Sigmoid()(rewards) > 0.5).to(labels) == labels).float().mean()
        return loss, acc
